...
K_FOLD_JOBS=2                              # Runs jobs for folds 0 to 2 (3-fold CV)
...
NODE_LIST=gpu-9,gpu-7,gpu-8,gpu-14    # List of nodes that the job can run on gpu-9,gpu-7,gpu-8,gpu-14 (h100)
...
OLLAMA_PORT="11444"                       # Remote port where Ollama listens. If different parallell runs, change ollama_port to avoid conflicts if same node is allocated.
...
if [ -n "$PROMPT_TYPE" ]; then
    REMOTE_DIR="/fp/homes01/u01/ec-sindrre/slurm_jobs/${EXPERIMENT}/${PHASE}/${CONTEXT_TYPE}/${PROMPT_TYPE}"
else
    REMOTE_DIR="/fp/homes01/u01/ec-sindrre/slurm_jobs/${EXPERIMENT}/${PHASE}/${CONTEXT_TYPE}"
fi
...
# Job Configuration
#SBATCH --job-name=${PHASE}_${EXPERIMENT}_${CONTEXT_TYPE}_${PROMPT_TYPE}_${GPUS}        # Job name
