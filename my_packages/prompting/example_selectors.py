...
# # Replace the following with your chosen embeddings class:

# def build_deterministic_faiss(embedding, texts: list[str]) -> FAISS:
#     """
#     Build a deterministic FAISS vector store using IndexFlatL2.
    
#     Parameters:
#       embedding: An instance of an Embeddings class (should be deterministic).
#       texts: A list of strings to be stored in the vector store.
      
#     Returns:
#       A FAISS vector store instance configured for deterministic behavior.
#     """
#     # Force determinism by setting fixed seeds:
#     np.random.seed(42)
#     random.seed(42)
#     # Limit FAISS to one thread:
#     faiss.omp_set_num_threads(1)
    
#     # Compute embeddings for each text.
#     # (Assumes embedding.embed_documents returns a list of embeddings.)
#     embeddings = embedding.embed_documents(texts)
#     embeddings = np.array(embeddings).astype("float32")
#     d = embeddings.shape[1]
    
#     # Create a deterministic FAISS index using IndexFlatL2.
#     index = faiss.IndexFlatL2(d)
#     index.add(embeddings)
    
#     # Build the docstore: a mapping of document IDs (as strings) to Document objects.
#     docs = {str(i): Document(page_content=text) for i, text in enumerate(texts)}
#     docstore = InMemoryDocstore(docs)
    
#     # Build a mapping from FAISS index positions to document IDs.
#     index_to_docstore_id = {i: str(i) for i in range(len(texts))}
    
#     # Instantiate the FAISS vector store.
#     # Note: The order of parameters is important!
#     vectorstore = FAISS(
#         embedding,            # embedding_function (or Embeddings instance)
#         index,                # FAISS index
#         docstore,             # Docstore object
#         index_to_docstore_id, # Mapping from index positions to doc IDs
#         normalize_L2=False    # (Optional) normalization flag
#     )
#     return vectorstore
